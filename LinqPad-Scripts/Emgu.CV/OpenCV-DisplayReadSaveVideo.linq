...
void Main()
{
	...
	string outputFilepath = @"F:\projects_csharp\OpenCV-practice\race_car_copy.mp4";
	...
	try
	{
	    ...
	    writer = new VideoWriter(
	        outputFilepath,
	        VideoWriter.Fourcc('M', 'J', 'P', 'G'), // Codec (MJPG)
	        fps,
	        new System.Drawing.Size(frameWidth, frameHeight),
	        isColor: true
	    );
        ...
        while (true)
        {
			...
			if (CvInvoke.WaitKey(waitTimeInt) == 27) // Exit if 'Esc' key is pressed
            {
                break;
            }
        }
